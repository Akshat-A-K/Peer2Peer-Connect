...
void download_file(int tracker_fd, const string &args)
{
    ...
    while(1)
    {
        ...
        if (n <= 0) { cout << "Tracker read error\n"; return; }
        resp.append(buf, n);
    ...
    if (!getline(riss, line)) { cout << "Unexpected tracker response (empty)\n"; return; }

    if (!line.empty() && line.back() == '\r') line.pop_back();
    istringstream h(line);
    string token;
    long filesize; string sha_full; int num_pieces; string filepath_on_peer;
    ...
    if (!getline(riss, line)) { cout << "Malformed tracker response: missing piece hashes\n"; return; }
    if (!line.empty() && line.back() == '\r') line.pop_back();
    vector<string> piece_hashes;
    {
        istringstream phs(line); string ph;
        while (phs >> ph) piece_hashes.push_back(ph);
    }

    if (!getline(riss, line)) { cout << "Malformed tracker response: missing peers\n"; return; }
    if (!line.empty() && line.back() == '\r') line.pop_back();
    vector<string> peers;
    {
        istringstream ps(line);
        string first;
        if (!(ps >> first)) { /* no peers */ }
        else {
            if (first == "PEERS") {
                string p; while (ps >> p) peers.push_back(p);
            } else {
    ...
    if (peers.empty()) { cout << "No peers available\n"; return; }
    ...
    if (fd < 0) { cout << "Failed to create part file at " << partname << "\n"; return; }

    // Prepare piece queue
    queue<int> pending_pieces;
    for (int i = 0; i < task->num_pieces; i++) pending_pieces.push(i);
    mutex queue_mtx;
        ...
        while (true) {
            ...
            while (!piece_done && retries < 5 && !task->cancel.load()) {
                for (size_t pi = 0; pi < peers.size() && !piece_done; ++pi) {
                    ...
                    if (send_all(s, req.c_str(), req.size()) < 0) { close(s); continue; }

                    string hdr;
                    if (!recv_line(s, hdr)) { close(s); continue; }
                    if (hdr.rfind("DATA", 0) != 0) { close(s); continue; }

                    size_t len = 0;
                    { istringstream hh(hdr); string tmp; hh >> tmp >> len; }

                    vector<char> pbuf(len);
                    if (recv_all(s, pbuf.data(), len) != (ssize_t)len) { close(s); continue; }
                    close(s);

                    // Compute SHA of received piece
                    string got = sha1_buf((unsigned char*)pbuf.data(), len);
                    if (got != task->piece_hashes[piece]) { retries++; continue; }

                    // Determine correct offset
                    off_t offset = (off_t)piece * (512 * 1024);
                    ssize_t wr = pwrite(fd, pbuf.data(), len, offset);
                    if (wr != (ssize_t)len) { retries++; continue; }
    ...
    vector<thread> pool;
    for (int i = 0; i < max_threads; i++) pool.emplace_back(worker, i);
    ...
    if (fd2 < 0) { cout << "Open assembled file failed\n"; return; }
    SHA_CTX ctx; SHA1_Init(&ctx);
    ssize_t rr;
    while ((rr = read(fd2, buf, sizeof(buf))) > 0) SHA1_Update(&ctx, buf, rr);
    close(fd2);

    unsigned char finalhash[SHA_DIGEST_LENGTH];
    SHA1_Final(finalhash, &ctx);
    char finalhex[SHA_DIGEST_LENGTH * 2 + 1];
    for (int i = 0; i < SHA_DIGEST_LENGTH; i++) sprintf(finalhex + i * 2, "%02x", finalhash[i]);
