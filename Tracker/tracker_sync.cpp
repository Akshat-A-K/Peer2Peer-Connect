    ...
    thread([peer_ip, peer_port]()
        ...
        while(1) 
        {
            ...
            while(1) 
            {
                ...
                if(tokens.size()==0)continue;else{
                    if(tokens[0]=="create_user")create_user(tokens);
                    else if(tokens[0]=="create_group")create_group(tokens,-1);
                    else if(tokens[0]=="join_group")join_group(tokens,-1);
                    else if(tokens[0]=="accept_request")accept_request(tokens,-1);
                    else if(tokens[0]=="leave_group")leave_group(tokens,-1);
                        ...
                        if(tokens.size()<6){}
                            ...
                            int idx=6;
                            for(int i=0;i<num_pieces && idx<(int)tokens.size();++i,++idx)piece_hashes.push_back(tokens[idx]);

                            unordered_set<string> shared_by;
                            if(idx<(int)tokens.size()&&tokens[idx]=="SHARED_BY"){++idx;while(idx<(int)tokens.size())shared_by.insert(tokens[idx++]);}
                            ...
                            {lock_guard<mutex> lock(files_mutex);group_files[group_id].push_back(f);all_files[filepath]=f;}
                        }
                    }
                        ...
                        if(tokens.size()>=4){
                            ...
                            if(group_files.count(group_id)){
                                for(auto &gf:group_files[group_id])if(gf.filename==filename)gf.shared_by.erase(peerid);
                                for(auto it=all_files.begin();it!=all_files.end();){
                                    if(it->second.filename==filename&&it->second.group_id==group_id){
                                        if(it->second.shared_by.empty())it=all_files.erase(it);else++it;
                                    }else++it;
                                }
                                auto &vec=group_files[group_id];
                                vec.erase(remove_if(vec.begin(),vec.end(),[&](const FileInfo &fi){return fi.filename==filename&&fi.shared_by.empty();}),vec.end());
                            }
                        }
                    }
