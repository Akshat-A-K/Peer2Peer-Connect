...
string compute_sha1(const string& filepath) 
{
    ...
    if(bytes_read==-1) { close(fd); return ""; }
...
string upload_file(const vector<string>& tokens, int client_fd)
{
    ...
    if(filesize==-1) return "File does not exist on disk";

    string full_hash=compute_sha1(filepath);
    if(full_hash.empty()) return "Error computing SHA-1";
    ...
    if(client_user.count(client_fd)) {
        ...
        if(user_ports.count(username)) peerid = user_ports[username];
        else peerid = username;
    }
    ...
    for(auto &ff: group_files[group_id])
    {
        if(ff.filename==filename)
        {
            ...
            for(auto &ph: piece_hashes) m+=" "+ph;
            m += " SHARED_BY";
            for(auto &s: ff.shared_by) m += " " + s;
...
{
    if(tokens.size()!=3) return "Usage: stop_share <group_id> <file_name>";
        ...
        if(it->second.filename==filename && it->second.group_id==group_id)
        {
            ...
            if(f.shared_by.empty())
            {
                ...
                vec.erase(remove_if(vec.begin(), vec.end(),
                                    [&](const FileInfo &fi){ return fi.filename==filename; }),
                          vec.end());
                ...
                for(auto &gf: group_files[f.group_id]) {
                    if(gf.filename==filename) gf.shared_by = f.shared_by;
                }
            }
